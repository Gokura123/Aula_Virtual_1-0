...
function limpiarSesion() {
    ...
    // localStorage.removeItem('entregas'); // ✨ COMENTADO: Preservar entregas para mantener estado "Entregado"
...
async function cargarTareasEstudiante() {
    ...
    // ✨ NUEVA LÍNEA: Sincronizar entregas después de cargar tareas
    await sincronizarEntregasEstudiante();
}
...
// Función para sincronizar entregas del servidor con localStorage (solo para estudiantes)
async function sincronizarEntregasEstudiante() {
    if (!usuarioActual || usuarioActual.tipo !== 'estudiante') return;
    
    try {
        console.log('🔄 Sincronizando entregas del servidor...');
        const response = await fetch('/api/entregas');
        const result = await response.json();
        
        if (result.success && result.entregas) {
            // Filtrar solo las entregas del estudiante actual
            const entregasEstudiante = result.entregas.filter(entrega => 
                entrega.estudianteNombre === usuarioActual.nombre
            );
            
            // Actualizar localStorage con las entregas reales del servidor
            const entregasParaLocalStorage = entregasEstudiante.map(entrega => ({
                tareaId: entrega.tareaId,
                estudiante: entrega.estudianteNombre,
                fecha: entrega.fechaEntrega
            }));
            
            localStorage.setItem('entregas', JSON.stringify(entregasParaLocalStorage));
            console.log(`✅ Sincronizadas ${entregasParaLocalStorage.length} entregas del servidor para ${usuarioActual.nombre}`);
        }
    } catch (error) {
        console.error('Error al sincronizar entregas:', error);
    }
}
